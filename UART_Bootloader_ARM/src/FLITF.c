...
void Flash_voidEraseAppArea(void)
{
	u8 iterator;

	for (iterator=7;iterator<64;iterator++)
	{
		Flash_ErasePage(iterator);
	}
}
...
void Flash_ProgramWrite(void * StartAddress,void * DataAddress,uint_32t DataLength)
{
	...
	{
		MY_Temp = (( uint_16t *) DataAddress )[Iterator];
		(( uint_16t *) DataAddress )[Iterator]= (( (0x00FF) & (( ( uint_16t *) DataAddress )[Iterator])) << 8);
		MY_Temp=((MY_Temp>>8)&(0x00FF));
		(( uint_16t *) DataAddress )[Iterator]=(( uint_16t *) DataAddress )[Iterator]|((u16)MY_Temp);
		((volatile uint_16t *) StartAddress )[Iterator]  = ( ( uint_16t *) DataAddress )[Iterator];
...
/*
 void Flash_ProgramWrite2(volatile void * StartAddress, volatile void * DataAddress, u16 NumberOfBytes)
 {
  	u16 Local_Iterator = 0;

	volatile u32 * pptr = (volatile u32)(&StartAddress);

	while(FPEC->FLASH_SR & 0x00000001);
	FPEC->FLASH_CR |= 0x00000001;

	for(Local_Iterator=0; Local_Iterator<NumberOfBytes; Local_Iterator++)
	{
		*( ((u16 *)StartAddress) + Local_Iterator) =(u16) *( ((u16*)DataAddress) + Local_Iterator) ;

		while(FPEC->FLASH_SR & 0x00000001);
		FPEC->FLASH_CR |= 0x00000020;
